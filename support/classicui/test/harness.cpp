...
static void assert_disc_launch()
{
	...
	/*
	  ...
	  is exactly what the PSX block proves - the right core, the sentinel, the right
	  slot - so the checks are the same three.
	*/
	// The PSX launch closed the UI and latched the drive to its core; come back the
	// way the cleanup below comes back - leave, then the menu key - so this case
	// starts from the shelf like a player would. The latch clears itself once the
	// loop sees the menu core again.
	disc_reset_reader();
	disc_ingest_present(0);
	(void)disc_take_dirty();
	chome_leave();
	press(KEY_MENU, 20);
	frame(10);
	check(chome_screen_id() == S_HOME, "the shelf is back after the PSX launch");
