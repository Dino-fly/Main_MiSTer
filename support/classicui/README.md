...
Nineteen presets exist, but **the list is never shown in full**: the Display screen
...
is never offered an LCD; a Game Boy game is never offered a PVM. Every class ends
with one entry that switches all processing off - Sharp for the CRT classes, None
for the handhelds.

A look drives **two machines**. The scaler half is written out as a real MiSTer
preset file and applied with `video_loadPreset(path, save=true)`, which persists
per core. The core half is a list of the core's **own** options set by name
("Screen Shadow=Yes", "Modify Colors=GBA 2.2") plus, on the Game Boy, a real
`.gbp` palette pushed at the core's palette slot. A core that does not publish an
option is simply left alone, and nothing the looks set is ever saved into
`<CORE>.CFG` - a game launched from the stock menu keeps the player's own setup.

The split follows who does each job better. The Game Boy core colourises DMG
games **by default** (its `Custom Palette=Auto` fallback is an olive/teal set
baked into the RTL), and the GBA core carries the Pokefan531 colour correction
per screen model; a scaler gamma LUT on top of either is double processing - that
was exactly the too-green, too-dark DMG this replaced. So colour belongs to the
core, and the scaler keeps only what a core cannot do: the pixel grid, drawn by a
polyphase filter whose gutter is indexed by position *inside* each source pixel,
so it lands on core pixel boundaries at any scale and offset. (A shadow mask
repeats in output pixels and cannot promise that - 1080p denies the Game Boy both
the integer scale and a divisible offset.) The LCD looks also pin the core's own
`Scale` to HV-integer so the grid cells stay equal.
...
| `shadow_masks/ClassicHome Dot Matrix.txt` | 4x4 LCD cell grid, still used by the colour handhelds |
| `gamma/ClassicHome Game Gear / Lynx / WonderSwan / NGPC ... .txt` | 256-entry per-channel LUTs, only for the panels whose cores offer no colour work of their own |
| `games/GAMEBOY/Palettes/ClassicHome DMG Green / Pocket.gbp` | Real Game Boy palettes - bgb green and a neutral grey - in the core's own format |
| `presets/ClassicHome *.ini` | One per look, plus a `(CRT)` twin of each handheld look with the structure lines dropped - see below |

A generated file is brought up to date when a build changes its content; a file
without the "generated by Classic Home" marker (for `.gbp`, the `CH` tail bytes)
...
### On an analog display, only the colour half applies

A handheld look divides into **colour** and **structure**, and the division is data -
two columns of the preset table, `core_colour` and `core_struct` - rather than a
special case in the apply.

| | What it is | Where it applies |
|---|---|---|
| Colour | The `.gbp` palette, `GBC Colors`, `Modify Colors`, and the gamma LUTs for the panels whose cores offer no colour work | Any output. A Game Boy on a CRT wants DMG green as much as one on HDMI |
| Structure | The pixel grid, `Screen Shadow`, `Frame blend` / `Flickerblend`, and the HV-integer `Scale` that exists only to keep the grid's cells equal | The scaler path only. On a real tube a simulated LCD structure is not a simulation of anything, it is damage |

The test is `vp_output_is_analog()`: **no HDMI sink attached**, so whatever the
routing - `direct_video`, `vga_scaler`, or the front-end's own takeover - a
television or a CRT is what the player is looking at. It is deliberately not
`video_scaler_is_visible()`, which answers the different question of whether the
scaler output reaches a screen: with `vga_scaler=1` the scaler is very much in the
path and pointing down a VGA cable at a tube, which is the one setup where the grid
filter would be drawn onto real phosphors.

The same split runs through the scaler half, which is why `vp_install()` writes a
`(CRT)` twin of each handheld preset with the filters and mask forced off and the
gamma kept. It is loaded *instead of* the look's own file, never as well as, so a
session that starts on HDMI and moves to a CRT clears the grid out of the scaler
rather than leaving it there unseen.

Consequently the **Display screen is reachable for a handheld on an analog set**,
including at 240p where it is otherwise dropped: the palette is the one thing that
still works there and the one thing the player has no other way to reach. The list
is shortened to the looks that can still show a difference, so the Game Gear, Lynx,
WonderSwan and NGPC - whose entire colour work is a scaler gamma LUT - are offered
on `vga_scaler` and not on `direct_video` or the takeover, where the LUT is not in
the path. A class left with nothing but its off switch gets no Display entry at all,
which is the same rule as listing no look that would do nothing.

An output that changes under a running game is noticed by `vp_output_poll()`, once
the launch's look has landed: both halves are then applied again for the output that
is really there, and the undo records (`vp_origs`) put the structure options back to
**the player's own** values rather than to any table of defaults.
...
analog-only set they change nothing at all. (The Display entry has since gained one
exception to that test - a handheld's palette is the core's own and works on any
output. See [On an analog display](#on-an-analog-display-only-the-colour-half-applies).
These options have no such half: every one of them is scaler work.)
